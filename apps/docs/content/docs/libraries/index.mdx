...
a copy of your workbook. Passing `preserveWorkbook: false`, `range`, or `sheet`
selects a compact mode instead, which rebuilds a single worksheet from the
grouped values and carries none of these structures — no other worksheets, no
charts, no macros — so do not quote these cells at a caller who selected one of
those modes.
