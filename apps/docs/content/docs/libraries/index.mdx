...
The suggestion uses no similarity scoring and no sample values, so it has no
false positives — and equally, it never guesses. Synonyms that share no
spelling, such as `Timestamp` and `Run Time`, stay for you to add by hand. A
drafted entry carries no aliases on purpose: every spelling in a group shares
one normalized key, which the canonical name already matches. Review the draft
and apply it yourself; nothing is applied for you.
