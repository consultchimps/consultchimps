...
worksheets with their visibility and dimensions, the header row each operation
would actually use, Excel Tables, named ranges, and a bounded sample of every
column's values. Use it to confirm a workbook's shape, or to build a picker for
the sheets and columns another operation will act on.
...
Each sheet also reports `uncachedFormulaCells`: cells in the region an operation
would read, its header row included, that hold a formula the workbook carries no
calculated value for. The header row counts because a header that was never
calculated reads as blank, and a column with no header is not read under a
missing name but under an invented one. Excel writes a formula and its last
result side by side, but a file written by a generator, or saved with
calculation switched off, carries the formula alone. Every reader then sees
those cells as empty, because empty is all the file says, so this count is the
only way to tell them from cells that really are empty. Calculating them belongs
to Excel: open the workbook, let it calculate, and save it again.
