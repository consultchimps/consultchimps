...
A cell Excel stores as a date reads as ISO 8601 text, and that text is a
function of the workbook alone. Excel holds a date as a count of days from the
workbook's epoch, wearing a date number format, and attaches no time zone, so
the reader decodes that count into calendar components and writes them out. No
`Date` takes part: a `Date` has a local face as well as a UTC one, and text
built from the local face makes the same workbook read as a different calendar
day in every time zone. The same workbook therefore reads identically on a
machine set to UTC, to UTC+4, or to UTC-7, which is what a group key, an output
filename, and a stored date all need.
