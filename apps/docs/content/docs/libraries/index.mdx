...
an offset of `+99:99` is not an offset. A date written any other way
(`01/02/2026`) stays text, because there is no way to tell a January date from a
February one without guessing a convention the file never stated. So does a
column whose values disagree: `1` beside `yes` is text.
