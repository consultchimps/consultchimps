...
fingerprint binds reviewed metadata, including the recipe, decisions, source
...
artifact format 3 rejects version 1 and 2 spike plans; regenerate those plans
from their original sources. Use
...
bridges the streaming Excel reader into this pipeline. Applying a plan validates
positive safe source-row coordinates and captured row counts before committing
provenance. Stored byte and row counts must be nonnegative safe integers;
capture reuse flags must be stored as 0 or 1. Invalid prepared rows return
`DB_INVALID_PREPARED_IMPORT`; invalid reused database captures return
...
Opening managed databases and prepared plans validates internal column order,
...
Opening checks source, capture, import, and delivery counters against their
stored identifiers. A new table application checks row counters against the
latest allocated row in each table, then checks its pending Record IDs for
collisions with existing records, including records without import provenance.
The row checks run inside the write transaction and are skipped for
already-applied routes and read-only opening. SQLite can use the existing
imported-row index; DuckDB checks the numeric imported-row column. These checks
allow gaps and return `DB_CORRUPT_DATABASE` on a conflict without repairing
counters. Preserve generated Record IDs and provenance when editing row values
externally. This is not a full audit of arbitrary external Record ID edits.
...
In the browser, `BrowserDatabaseRuntime.listPreparedImports` returns readable
`imports` and structured `ignored` entries containing `name`, `code`, and
`message`. An unsupported saved-plan version is reported without deleting it.
...
by a standalone delivery with `DB_REQUEST_ID_CONFLICT`. `recordDelivery` cannot
create a delivery under an import-only request ID. When an import also records a
delivery, `recordDelivery` can return that existing event if its context and
capture membership match. It does not add another event or change the import
receipt.

`applyImport` checks that the approved plan belongs to the target database
before returning a saved request receipt. Cross-format conversion copies the
history but assigns a new database ID, so replaying the source plan returns
...
after review invalidates the schema or import plan before table writes.

The applications in a retry receipt must match the reviewed capture and
destination routes, including repeated routes. Their original request and plan
...
locations if temporary cleanup fails. `importDatabase` and
`createPreparedImport` return handles rather than operation results, so they
instead report `DB_BROWSER_PUBLICATION_CLEANUP_REQUIRED` with `published: true`.
The public working copy remains stored. If `handleCloseFailed` is false, reopen
the reported public `name` to continue; otherwise release its handles first.
...
Call `discardPreparedImport({ name })` to remove a closed private plan that the
...
explains recipe, schema, delivery, and browser-copy semantics.
