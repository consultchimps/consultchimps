...
| `date`    | every value is an ISO 8601 date or date and time, on the calendar |
| `integer` | every value is a whole number, exactly representable              |
| `real`    | every value is a number, a fractional part allowed                |
| `text`    | anything else, and a column with no values in it                  |

Nothing is guessed. A padded reference code (`007`), an exponent form (`1e5`), a
grouped number (`1,000`), a whole number past the range JavaScript represents
exactly, and a date written any other way (`01/02/2026`) all stay text, because
reading them as a number or a date would change them. So does a column whose
values disagree: `1` beside `yes` is text.
