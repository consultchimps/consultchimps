...
The output name has to agree with the workbook's declared type: an ordinary
workbook keeps `.xlsx` and a macro-enabled workbook keeps `.xlsm`. Naming the
output the other way is refused with `XLSX_UNPROTECT_PACKAGE_TYPE_MISMATCH`
before anything is written, because a package whose contents and name disagree
is one Excel opens with a corruption warning. `unprotectWorkbookBytes` is the
byte-level twin, taking and returning in-memory bytes for callers with no
filesystem.
