...
bindings, and capture definitions, so a changed mapping cannot reuse an earlier
approval. It does not authenticate the artifact or its captured rows. Prepared
artifact format 2 rejects version 1 spike plans; regenerate those plans from
their original sources. Use
