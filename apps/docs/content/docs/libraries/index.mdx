...
`UNDECIDED_DESCRIBE_STRUCTURES` record why each one is still open. Use the table
to warn people before a run, or read it as prose in
[what the Excel operations preserve](/docs/tools/excel-preservation), which is
generated from it and spells out the same limits.
