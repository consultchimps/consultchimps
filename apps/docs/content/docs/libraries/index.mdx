...
Each sheet also reports `uncachedFormulaCells`: cells below the header row that
hold a formula the workbook carries no calculated value for. Excel writes a
formula and its last result side by side, but a file written by a generator, or
saved with calculation switched off, carries the formula alone. Every reader
then sees those cells as empty, because empty is all the file says, so this
count is the only way to tell them from cells that really are empty. Calculating
them belongs to Excel: open the workbook, let it calculate, and save it again.
