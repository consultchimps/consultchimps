...
<Callout title="What the online tool does">
  ...
  tool. The browser accepts `.xlsm` workbooks as well. A whole-workbook split
  preserves the package, so `clients.xlsm` gives you `clients-North.xlsm` with
  the macro project carried into each one; narrowing to a table, a range, or a
  worksheet rebuilds compact workbooks instead, which are always `.xlsx` and
  carry no macro project. A package whose type contradicts its name — a
  macro-enabled workbook renamed `.xlsx`, or an ordinary one renamed `.xlsm` —
  is refused with the same explanation the command line gives, before any file
  is offered.
</Callout>
...
links, ActiveX controls, unsupported extension parts, chart source ranges,
defined names, and formulas that depend on removed rows are preserved but are
not semantically rewritten or recalculated. Review complex workbooks in Excel
before delivery.

[What the Excel operations preserve](/docs/tools/excel-preservation) is the
full, structure-by-structure table, generated from the conformance contract
these operations are tested against.
