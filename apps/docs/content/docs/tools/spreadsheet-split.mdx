...
<Callout title="What the online tool does">
  ...
  tool. The browser accepts `.xlsm` workbooks as well and keeps the extension,
  so `clients.xlsm` gives you `clients-North.xlsm` with the macro project
  carried into each one; a package whose type contradicts its name — a
