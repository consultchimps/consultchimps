...
[What the Excel operations preserve](/docs/tools/excel-preservation) sets these
promises out structure by structure, beside what the split and the workbook
inspection do with the same structures.
...
output filename you type decides the type, so end it in `.xlsm` to keep a macro
project that only one source carries, and leave it alone for an `.xlsx`.
Consolidation has [its own guide](/docs/tools/spreadsheet-consolidate) and
