...
as that row, or at most one value fewer, is the header. Rows above it are title
rows only when a blank row sets them apart from the header or they are banners
merged across the columns, such as a report title merged across the table or a
"Prepared by" line with a blank row under it. Those rows are skipped, and the
result reports how many were.
