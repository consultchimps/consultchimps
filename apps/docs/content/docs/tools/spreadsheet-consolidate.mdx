...
<Callout title="What the online tool does">
  ...
  output is always an `.xlsx`. The work happens in a Web Worker inside the page,
  so your workbooks never leave your machine; progress is reported while a run
  is in flight, and a long run can be cancelled.
</Callout>
...
<Callout title="Mapping is a command-line feature today">
  The [in-browser consolidator](/tools/excel-consolidate) does not offer column
  mapping or mapping suggestions. Both are available through the command line
