...
Input files are untrusted, and both the report and the progress lines are
written to a terminal, so a control character in a worksheet name, header, or
value is printed as a visible `\uXXXX` escape instead of reaching the terminal
as an instruction. Sample values and Excel Table column names are quoted, with a
quote inside one escaped and a backslash doubled, so one value can never read as
two. The values themselves are unchanged: `--json` reports what the workbook
holds.
