...
Options:
  ...
  --no-preserve-workbook    create a compact data-only workbook for a table split
  --values                  replace formulas with stored values, preserving formatting
...
result becomes a formatted blank cell. Consolidation and compact data-only split
outputs already contain values, so the option is explicit but does not otherwise
change those files.
