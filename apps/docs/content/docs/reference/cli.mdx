...
- `db inspect --limit <number>` limits the saved plan's row preview to 20 by
  default. Database inspection reports table and capture summaries.
...
- `--input <source>`: repeat for several workbooks. Use `alias=path` to keep
  recipe references independent of filenames. A plain path uses its filename
  stem.
...
accepts `--recipe` for changed routing or mappings. Omit the recipe to re-review
