...
a terminal is an instruction rather than a character. So wherever text from a
document reaches the terminal, whether it is a worksheet name quoted in a
warning, the path of a created file, a progress line, or the name in an error
message, its control characters are printed as a visible `\uXXXX` escape. The
values are unchanged: `--json` reports the original text, where JSON's own
escaping already makes a control character inert.
