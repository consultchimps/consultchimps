...
with lowercase `x` and four uppercase hex digits. Encode every XML 1.0-forbidden
UTF-16 code unit and every carriage return as `_xHHHH_`. This includes forbidden
controls, U+FFFE, U+FFFF, and unpaired surrogates; preserve valid surrogate
pairs as their Unicode character. Protect an original literal `_xHHHH_`, with
either case of hex digits, by encoding its leading underscore as `_x005F_`; do
not re-escape sequences generated by this pass. Thus literal `_x000A_`
serializes as `_x005F_x000A_`, while an actual line feed remains a line feed in
element text. Apply ordinary XML entity escaping as well, and preserve leading
and trailing whitespace. The workbook writer owns this encoding; callers pass
logical strings and must not pre-escape them.
...
controls, U+FFFE, U+FFFF, isolated high and low surrogates, CR/LF/CRLF, literal
escapes including `_x005F_`, XML metacharacters, edge whitespace, supplementary
Unicode, and truncation adjacent to these characters. Pin expected XML bytes as
well as read-back values so an escaping change cannot silently change the
...
Dates use the decoded logical IEEE-754 day serial, not nanosecond storage.
[Microsoft documents Date/Time as a decimal value](https://learn.microsoft.com/en-us/power-bi/connect-data/desktop-data-types#datetime-types),
and the
[reference decoder](https://github.com/Hugoberry/pbixray/blob/c5da940b45d2806c8f8d3e109f0d1909117a7db0/pbixray/vertipaq_decoder.py#L389)
converts day counts from 1899-12-30. The spike's sub-millisecond differences do
not establish nanosecond precision in the source model. Preserve the decoded
double before any host date conversion. A non-null, non-finite date serial makes
the column unreadable under `PBI_COLUMN_UNREADABLE`.

Numeric date cells retain the proposed millisecond-rounding policy. Interpret
the finite serial's bits as an exact rational number, multiply by 86,400,000
milliseconds per day using exact arithmetic, and round once to an integer.
Resolve an exact half millisecond toward the later instant. Do not first round
to nanoseconds, host date precision, or the documented 1/300-second model time
granularity. Count an emitted numeric date as `PBI_DATE_ROUNDED` only when that
integer differs from the exact product; nulls and unchanged values do not count.

Use the workbook's 1900 date system. Check the original serial first: values
below 2, meaning before 1900-01-01, select text without rounding, even if
rounding would cross that boundary. Otherwise form the rounded candidate from
the integer milliseconds since 1899-12-30. For dates before 1900-03-01 subtract
one from the corresponding day count to obtain the Excel serial; from that day
onward the day counts agree. Never generate Excel's fictitious 1900-02-29. Write
a numeric date only when the candidate remains within years 1900 through 9999
and the serialized cell reads back to the same integer millisecond through the
independent workbook reader. A candidate in year 10000 or any failed round trip
selects text instead.

Date fallback text preserves the original serial as `pbi-date-serial:` followed
by ECMAScript `Number::toString` in radix 10, with an explicit `-0` for negative
zero. This canonical shortest decimal reconstructs the same finite double; it
avoids inventing an ISO timestamp with rounded fractional precision. For
example, serial 1.5 becomes `pbi-date-serial:1.5`. The manifest identifies this
encoding and the 1899-12-30 epoch for affected date columns. Count these cells
as `PBI_DATE_AS_TEXT`, never also `PBI_DATE_ROUNDED`. Build items 7 and 8 test
serials on both sides of half-millisecond boundaries, pre-1900 values, the
January/February 1900 offset, the upper date limit, signed zero, and bit-exact
recovery of fallback serials. They also test a date-serial round trip crossing a
