...
- A declared date format is parsed into an ISO 8601 date string, and declared
  thousands and decimal separators into a number. A value that is not what its
  coercion declares is refused with `TABLE_MAPPING_COERCION_FAILED` naming the
  row and column; blank cells stay blank.
