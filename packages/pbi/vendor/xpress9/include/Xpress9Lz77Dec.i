...
/* Writes decoded data to the internal data buffer, m_pBufferData. Data is then copied from this buffer to the final buffer. */
    ...
    if (pDecoder->m_DecodeData.m_Tail.m_uLength != 0)
    {
        ...
        {
            SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "uDecodePosition=%u uBufferOffset=%I64u iOffset=%u", uDecodePosition, pDecoder->m_DecodeData.m_uBufferOffset, -iOffset);
            goto Failure;
        }
    ...
    do
    {
        ...
        if (uSymbol < LZ77_MTF)
        {
            ...
            if (iMtfLastPtr)
            {
                ...
                if (uSymbol != 0)
                {
                    SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "uDecodePosition=%u", uDecodePosition);
                    goto Failure;
                }
                iOffset     = iMtfOffset1;
#else
