...
)
{
    ...
    if (pDecoder->m_DecodeData.m_uMtfEntryCount > 0)
    {
        ...
        for (i = 0; i < pDecoder->m_DecodeData.m_uMtfEntryCount; ++i)
        {
            ...
            {
                SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "uMsb=%u MaxWindowSize=%u", uMsb, pDecoder->m_DecodeData.m_uWindowSizeLog2);
                goto Failure;
            }
...
//     12 bits      reserved, must be 0
    ...
    if (pDecoder->m_uState == LZ77_DECODER_STATE_WAIT_HUFFMAN)
    {
        ...
        {
            SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "HuffmanTableBits=%u DecodedHuffmanTableBits=%u", pDecoder->m_DecodeData.m_uHuffmanTableBits, uBytesToCopy);
            goto Failure;
        }
