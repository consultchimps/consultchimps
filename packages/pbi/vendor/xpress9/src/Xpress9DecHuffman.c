    ...
    *puSymbolCount = 0;
    ...
    do
    {
        ...
        if (puBitLengthTable[i] != 0)
        {
            ...
            RETAIL_ASSERT (n < HUFFMAN_CODEWORD_LENGTH_LIMIT, "ExcessivelyLargeBitLength: symbol=%u bits=%u", i, n);
...
)
{
    ...
    do
    {
        ...
        if ((n & 1) != 0) // n is odd
        {
            SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "i=%u n=%u", i, n);
            goto Failure;
        }
        n >>= 1;
        --i;
    ...
    {
        SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "n=%u Count[0]=%u", n, uBitLengthCount[0]);
        goto Failure;
    }
...
)
{
    ...
    if (k != 1)
    {
        SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "UnknownHuffmanTableEncoding=%u", k);
        goto Failure;
    }
    ...
    for (i = 0; i < HUFFMAN_ENCODED_TABLE_SIZE; ++i)
    {
        ...
        else
        {
            ...
            if (k >= uPrevSymbol)
            {
                ...
                if (k > 8)
                {
                    SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "PrevSymbol=%u CurrentSymbol=%u", uPrevSymbol, k);
                    goto Failure;
                }
            }
    ...
    for (i = 0; i < uAlphabetSize;)
    {
        ...
        default:
            ...
            {
                SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "i=%u iSymbol=%u uMaxCodewordLength=%u", i, iSymbol, uMaxCodewordLength);
                goto Failure;
            }
            puBitLengthTable[i] = (UInt8) iSymbol;
            ++i;
        ...
        case HUFFMAN_ENCODED_TABLE_ZERO_REPT:
            ...
            if (k == 3)
            {
                do
                {
                    ...
                    {
                        SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "i=%u iSymbol=%u uAlphabetSize=%u uFillBoundary=%u", i, iSymbol, uAlphabetSize, uFillBoundary);
                        goto Failure;
                    }
                }
                while (k == 7);
...
//printf (" %2u", puBitLengthTable[i-1]); if ((i & 15) == 0) printf ("\n");
        ...
        case HUFFMAN_ENCODED_TABLE_ROW_0:
            if (i < uFillBoundary)
            {
                SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "i=%u uFillBoundary=%u iSymbol=HUFFMAN_ENCODED_TABLE_ROW_0", i, uFillBoundary);
                goto Failure;
            }
            puBitLengthTable[i] = puBitLengthTable[i - uFillBoundary];
            if (puBitLengthTable[i] == 0)
            {
                SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "i=%u iSymbol=HUFFMAN_ENCODED_TABLE_ROW_0 Length=0", i);
                goto Failure;
            }
...
//printf (" %2u", puBitLengthTable[i-1]); if ((i & 15) == 0) printf ("\n");
        ...
        case HUFFMAN_ENCODED_TABLE_ROW_1:
            if (i < uFillBoundary)
            {
                SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "i=%u uFillBoundary=%u iSymbol=HUFFMAN_ENCODED_TABLE_ROW_1", i, uFillBoundary);
                goto Failure;
            }
            puBitLengthTable[i] = (UInt8) (puBitLengthTable[i - uFillBoundary] + 1);
            if (puBitLengthTable[i] > uMaxCodewordLength || puBitLengthTable[i] == 0)
            {
                SET_ERROR (Xpress9Status_DecoderCorruptedData, pStatus, "i=%u iSymbol=HUFFMAN_ENCODED_TABLE_ROW_1 Length=%u", i, puBitLengthTable[i]);
                goto Failure;
            }
            uPrevSymbol = puBitLengthTable[i];
            ++i;
