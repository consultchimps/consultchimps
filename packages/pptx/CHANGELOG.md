...
- cc4d06c: Report a PowerPoint template inspection as a structured operation
  ...
  Both read whole packages before they can answer, so a caller that has moved
  on, such as a page replanning after a keystroke or inspecting a different
  slide, can stop that work rather than only discard its answer, and an
  inspection reports which of its two package reads a large deck is currently
  in.
